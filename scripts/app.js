...
function rerenderContent(activeHabbit) {
  ...
  for (const index in activeHabbit.days) {
    ...
    element.innerHTML = `<div class="habbit__day"><span>День ${
      Number(index) + 1
    } </span></div>
              ...
              <button class="habbit__delete" onclick="deleteDay(${index})">
                <img src="./images/delete.svg" alt="Удалить ${index + 1}" />
              </button>`;
    page.content.daysContainer.appendChild(element);
  }
  // Обновляем номер следующего дня в форме добавления комментария
  const formDayElement =
    document.querySelector(".habbit__form").previousElementSibling;
  if (formDayElement) {
    formDayElement.textContent = `День ${activeHabbit.days.length + 1}`;
  }
}
